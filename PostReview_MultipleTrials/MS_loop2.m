...
% Computes Delta (change in systolic pressure) and mean flow rate as functions lambda
% with s, R_a [unpressurized radius] and alpha [inlet pressure amplification factor] 
% as defined functions of lambda also
%
% size of lambda_vec
MMM = 7; 
...
% s, alpha and R_a are functions of lambda, phenomenologically specified:
s_vec = 0.85*lambda_vec; % s
pressure_ampl_factor_vec = 1 + 0.6*lambda_vec; % alpha
ref_rad_vec = 0.15 + 0.06*lambda_vec; % R_a

% s_vec = linspace(0,0.85,NNN);
% pressure_ampl_factor_vec = linspace(1,1.6,MMM);
% ref_rad_vec = linspace(0.15,0.21,MMM);

DELTA = zeros(MMM,1); MEAN_deltaP = zeros(MMM,1); MEAN_Q = zeros(MMM,1);
% MAX_Q = zeros(MMM,NNN); MEAN_A1 = zeros(MMM,NNN); MEAN_A2 = zeros(MMM,NNN);
...
for iii=1:length(lambda_vec)
        ...
        sprintf('iii = %d/%d, lambda = %d',iii,MMM,lambda_vec(iii))
        % modify WK parameters
        [~,~,~,~,~,~,Res0,Res,C,~,~,~,~,~,~,~] = LoadParameters(lambda_vec(iii),s);
        ...
        DELTA(iii) = max(P_set2_short) - max(P_set1_short); % difference in systolic pressures
        MEAN_deltaP(iii) = mean(P_set2_short) - mean(P_set1_short); 
        % MAX_Q(iii,jjj) = max(Q_set2_short);
        MEAN_Q(iii) = mean(Q_set2_short);
        % MEAN_A2(iii,jjj) = mean(A_set2_short); % mean area at outlet
        % MEAN_A1(iii,jjj) = mean(A_set1_short); % mean area at inlet
end

%%
subplot(1,4,1); plot(lambda_vec,s_vec,lambda_vec,pressure_ampl_factor_vec,lambda_vec,ref_rad_vec);
legend('s(\lambda)','\alpha(\lambda)','R_a');
xlabel('\lambda');

subplot(1,4,2); plot(lambda_vec,DELTA);
xlabel('\lambda'); ylabel('\Delta');

subplot(1,4,3); plot(lambda_vec,MEAN_deltaP);
xlabel('\lambda'); ylabel('Drop in Mean Pressure');

subplot(1,4,4); plot(lambda_vec,MEAN_Q);
xlabel('\lambda'); ylabel('Mean Flow Rate');
