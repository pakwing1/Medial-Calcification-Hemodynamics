...
% Compute colormaps as functions of lambda and R_a
% size of lambda_vec and ref_rad_vec
MMM = 11; 
NNN = 11;
...
ref_rad_vec = linspace(0.15,0.21,NNN);

s = 0.3;
pressure_ampl_factor = 1;
...
subplot(1,3,1); surf(lambda_vec,ref_rad_vec,DELTA');  title('\Delta_s (mmHg)'); colorbar; xlabel('\lambda'); ylabel('R_a (cm)'); view(2);
subplot(1,3,2); surf(lambda_vec,ref_rad_vec,MEAN_deltaP'); title('\Delta_m (mmHg)'); colorbar; xlabel('\lambda'); ylabel('R_a (cm)'); view(2);
subplot(1,3,3); surf(lambda_vec,ref_rad_vec,MEAN_Q'); title('Mean Outlet Flow Rate (cm^3/s)'); colorbar; xlabel('\lambda'); ylabel('R_a (cm)'); view(2);
