...
% Compute colormaps as functions of lambda and beta
% size of lambda_vec and beta
MMM = 11; 
NNN = 11;
...
pressure_ampl_factor_vec = linspace(1,1.6,NNN);

ref_rad = 0.15; % cm
s = 0.3; % closure fraction


DELTA = zeros(MMM,NNN); MEAN_deltaP = zeros(MMM,NNN); MEAN_Q = zeros(MMM,NNN);

for iii=1:MMM
    for jjj=1:NNN
        %% Load Parameters (except for WK parameters)
        [N,nm,T,L,x,P0,Res0,Res,C,rho,mu,...
            h,MechanicalParameters,tapering_factor,~,~] = LoadParameters(lambda_vec(iii),s);

        
        %% set up inlet and hydrostatic pressures
        P_in = LoadInPressure;
        P_in = @(t) pressure_ampl_factor_vec(jjj)*P_in(t);
        Pext = P_in(0);
        ...
        sprintf('iii = %d/%d, jjj = %d/%d',iii,MMM,jjj,NNN)            
...
subplot(1,3,1); surf(lambda_vec,pressure_ampl_factor_vec,DELTA');  title('\Delta_s (mmHg)'); colorbar; xlabel('\lambda'); ylabel('\alpha'); view(2);
subplot(1,3,2); surf(lambda_vec,pressure_ampl_factor_vec,MEAN_deltaP'); title('\Delta_m (mmHg)'); colorbar; xlabel('\lambda'); ylabel('\alpha'); view(2);
subplot(1,3,3); surf(lambda_vec,pressure_ampl_factor_vec,MEAN_Q'); title('Mean Outlet Flow Rate (cm^3/s)'); colorbar; xlabel('\lambda'); ylabel('\alpha'); view(2);
